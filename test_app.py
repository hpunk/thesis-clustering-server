...
def test_clustering_kmedoids():
    cluster_number = 3
    response = client.get('/clustering?st=15&pr=1501&di=150103&sd=2019-01-01&ed=2019-01-15&alg=0&k='+str(cluster_number)+'&eps=0&mins=0')
    assert response.status_code == 200
...
def test_clustering_dbscan():
    ...
    assert response.json()['response'][2]['group'] == '-1'
    assert response.json()['response'][1]['group'] == '1'
    assert response.json()['response'][0]['group'] == '0'
...
def test_clustering_hierarchical_scatter():
    number_clusters = 3
    response = client.get('/clustering/hierarchical-to-scatter?st=15&pr=1501&di=150103&sd=2019-01-01&ed=2019-01-15&alg=2&k='+str(number_clusters))
    assert response.status_code == 200
...
def test_sampling_of_clustering():
    count_response = client.get('/clustering/count?st=15&pr=1501&di=150103&sd=2019-01-01&ed=2019-03-30')
    data_response = client.get('/clustering?st=15&pr=1501&di=150103&sd=2019-01-01&ed=2019-03-30&alg=0&k=3&mins=0&eps=0')

    assert count_response.json()['count'] == 699
